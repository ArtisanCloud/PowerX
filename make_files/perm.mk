...
permgen.print: swagger.verify
	...
	@$(GO) run ./backend/cmd/perm_gen -openapi $(SWAG_JSON) -source $(SOURCE) -introduced $(INTRODUCED)

permgen.apply: swagger.verify
	@echo ">> APPLY from $(SWAG_JSON)"
	@$(GO) run ./backend/cmd/perm_gen -openapi $(SWAG_JSON) -source $(SOURCE) -introduced $(INTRODUCED) -apply

# ---- 基于“最小 OpenAPI”（/openapi.min.json）的权限同步 ----
.PHONY: permgen.min.print permgen.min.apply
permgen.min.print:
	@echo ">> dry-run from $(MIN_OPENAPI)"
	@$(GO) run ./backend/cmd/perm_gen -openapi $(MIN_OPENAPI) -source $(SOURCE) -introduced $(INTRODUCED)

permgen.min.apply:
	@echo ">> APPLY from $(MIN_OPENAPI)"
	@$(GO) run ./backend/cmd/perm_gen -openapi $(MIN_OPENAPI) -source $(SOURCE) -introduced $(INTRODUCED) -apply
