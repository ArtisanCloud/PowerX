...
func init() {
	...
	scanCmd.Flags().StringVar(&scanOpts.tenantID, "tenant-id", "", "Tenant identifier (required)")
	...
	_ = scanCmd.MarkFlagRequired("tenant-id")
...
func runVersionScan(cmd *cobra.Command, _ []string) error {
	...
	payload := map[string]string{
		"tenantId":       strings.TrimSpace(scanOpts.tenantID),
	...
	if err := client.do("POST", "/internal/version/governance/scan", payload, &report); err != nil {
		return err
	}
	out := cmd.OutOrStdout()
	fmt.Fprintf(out, "Tenant %s plugin %s -> current %s recommended %s\n",
		report.TenantID, report.PluginID, emptyFallback(report.CurrentVersion, "n/a"), emptyFallback(report.RecommendedVersion, "n/a"))
