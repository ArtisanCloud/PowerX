...
func TestRegisterAPIRoutes_LocalUploadUsesPresignedTokenWithoutAuthorization(t *testing.T) {
	...
	RegisterAPIRoutes(router, auth, cfg, &shared.Deps{})
	...
	require.False(t, authCalled, "presigned local upload route must not depend on user authorization")
	require.FileExists(t, filepath.Join(cfg.Storage.Local.BasePath, assetUUID, variant))
}

func TestRegisterAPIRoutes_LocalUploadRequiresUploadTokenSecret(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	router := gin.New()
	RegisterAPIRoutes(router, nil, cfg, &shared.Deps{})
