...
func platformCapabilityDefinitions() []platformCapabilityDefinition {
	platformCapabilityCacheOnce.Do(func() {
		...
		if len(defs) == 0 {
			if err == nil {
				log.Printf("[integration_gateway] no platform capability config found, using built-in defaults")
			}
...
func builtinPlatformCapabilityDefinitions() []platformCapabilityDefinition {
	return []platformCapabilityDefinition{
		...
		},
		{
			...
			Description:  "触发或控制 Workflow/Scheduler 实例，支持重试、暂停与继续。",
			Module:       "workflow",
			Categories:   []string{"workflow", "scheduler"},
			Intents:      []string{"workflow.scheduler.invoke"},
			ToolScopes:   []string{"workflow.scheduler"},
			Policy: capabilityPolicy{
				Prefer:   "grpc",
				Fallback: []string{},
			},
