...
2. 如果宿主已启用 CoreX 组件加载（Kernel Bootstrapping），系统会自动注册 Media Assets Management 模块的 HTTP 与 gRPC 接口；
3. 如需初始化数据库，请执行：`make migrate`。

## 核心验证流程

1. **上传媒体资源**
   - 调用 `POST {APIPrefix}/admin/media/assets`（默认 `{APIPrefix}=/api/v1`），表单或 JSON 指定上传方式。
   - 期望响应包含 `uuid`、`driver`、`businessStatus=draft`。
2. **查询与过滤**
   - 调用 `GET {APIPrefix}/admin/media/assets?keyword=logo&driver=s3&page=1&pageSize=20`。
   - 响应需返回 `items`、`total`、分页信息；软删除记录不可出现。
3. **更新业务属性**
   - 调用 `PATCH {APIPrefix}/admin/media/assets/{uuid}` 更新 `name`、`tags`、`businessStatus`。
   - 校验状态流转约束与审计日志。
4. **生成预签名链接**
   - 调用 `POST {APIPrefix}/admin/media/assets/{uuid}/presign` 指定 `action=download`。
   - 响应内包含 `url`、`method`，且 `expiresInSeconds=43200`。
5. **软删除与清理**
   - 调用 `DELETE {APIPrefix}/admin/media/assets/{uuid}`。
