...
func LoadLang() {
	...
	message.SetString(lang, fmt2.Sprintf("%d", config.API_ERR_CODE_REQUEST_PARAM_ERROR), "invalid request parameter")

	message.SetString(lang, fmt2.Sprintf("%d", config.API_ERR_CODE_FAIL_TO_GET_REPORT_LIST), "Failed to get the customer report list")
	message.SetString(lang, fmt2.Sprintf("%d", config.API_ERR_CODE_FAIL_TO_UPSERT_REPORT), "Failed to upsert the customer report")
	message.SetString(lang, fmt2.Sprintf("%d", config.API_ERR_CODE_FAIL_TO_DELETE_REPORT), "Failed to delete the customer report")
