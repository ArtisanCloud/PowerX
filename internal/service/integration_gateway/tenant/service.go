...
func (s *Service) ListRoutes(ctx context.Context, tenantID, capabilityID, channel string) ([]manager.Route, error) {
	...
	records, _, err := s.routes.ListByTenant(ctx, tenantID, 0, 0)
	if err != nil {
		return nil, err
	}
...
// GetRoute 返回路由详情。
func (s *Service) GetRoute(ctx context.Context, tenantID, slug string) (manager.Route, error) {
	tenantID = strings.TrimSpace(tenantID)
	if tenantID == "" {
		return manager.Route{}, errors.New("tenant_id required")
	}
	...
	record, err := s.routes.GetBySlug(ctx, tenantID, slug)
	if err != nil {
		return manager.Route{}, ErrRouteNotAccessible{Slug: slug, TenantID: tenantID}
	}
...
func (s *Service) Invoke(ctx context.Context, in InvokeInput) (result InvokeResult, err error) {
	...
	if tenantID == "" {
		return result, errors.New("tenant_id required")
	}
	...
	record, err := s.routes.GetBySlug(ctx, tenantID, slug)
	if err != nil {
		return result, ErrRouteNotAccessible{Slug: slug, TenantID: tenantID}
	}
	...
	if s.toolGrants != nil && len(route.ToolGrantIDs) > 0 {
		if err = s.toolGrants.Validate(ctx, tenantID, route.ToolGrantIDs); err != nil {
			...
			return result, ErrToolGrantDenied{RouteID: route.RouteID, TenantID: tenantID, Grants: route.ToolGrantIDs, Reason: err.Error()}
		}
	}
