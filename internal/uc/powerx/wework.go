...
func newWeWorkUseCase(conf *config.Config, db *gorm.DB, employee *OrganizationUseCase, department *DepartmentUseCase, auth *AuthUseCase, tag *TagUseCase) *WeWorkUseCase {
	// 初始化企业微信API SDK
...
func (s *WeWorkUseCase) FetchDepartments(ctx context.Context) {
	depResult, err := s.Department.List(ctx, 0)
	if err != nil {
...
func (s *WeWorkUseCase) FetchEmployees(ctx context.Context) {
	var deps []*WeWorkDepartment
	...
	for _, dep := range deps {
		userResult, err := s.User.GetDetailedDepartmentUsers(ctx, int(dep.ID), 0)
		if err != nil {
...
func (s *WeWorkUseCase) SyncDepartmentsToSystem(ctx context.Context) {
	var deps []*WeWorkDepartment
	if err := s.db.WithContext(ctx).Model(&WeWorkDepartment{}).Find(&deps).Error; err != nil {
...
func (s *WeWorkUseCase) SyncEmployeeToSystem(ctx context.Context) {
...
func (s *WeWorkUseCase) SyncDepartmentsLeadersToSystem(ctx context.Context) {
	var employees []*WeWorkEmployee
