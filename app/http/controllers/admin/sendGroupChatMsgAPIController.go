...
func APISendGroupChatMsgSync(context *gin.Context) {
	...
	rs, err := ctl.ServiceSendGroupChatMsg.SyncSendGroupChatMsgFromWXPlatform(global.DBConnection, startDatetime, endDatetime, limit, "")
	if err != nil {
...
func APIGetSendGroupChatMsgList(context *gin.Context) {
	...
	arrayList, err := ctl.ServiceSendGroupChatMsg.GetQueryList(global.DBConnection,
...
func APIGetSendGroupChatMsgDetail(context *gin.Context) {
	...
	sendGroupChatMsg, err := ctl.ServiceSendGroupChatMsg.GetSendGroupChatMsgByUUID(global.DBConnection, uuid)
	if err != nil {
...
func APIDoSendGroupChatMsgs(context *gin.Context) {
	...
	toSendList, err := ctl.ServiceSendGroupChatMsg.GetToDoSendList(global.DBConnection, &now, &endDatetime)
	if err != nil {
		ctl.RS.SetCode(config.API_ERR_CODE_FAIL_TO_GET_SEND_GROUP_CHAT_MSG_LIST, config.API_RETURN_CODE_ERROR, "", err.Error())
		panic(ctl.RS)
		return
