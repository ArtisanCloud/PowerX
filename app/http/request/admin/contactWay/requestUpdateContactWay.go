...
func convertParaToContactWayForUpdate(form ParaUpdateContactWay) (contactWay *models.ContactWay, updateTags []*wx.WXTag, err error) {
	...
	contactWay, err = contactWayService.GetContactWayByConfigID(database.DBConnection, form.ConfigID)
	if err != nil {
		return nil, nil, err
	}
	...
	updateTags, err = wxTagService.GetWXTagsByIDs(database.DBConnection, form.WXTagIDs)
	if err != nil {
		return nil, nil, err
	}
