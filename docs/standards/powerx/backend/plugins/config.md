...
database:
  driver: postgres
  dsn: postgres://pxu_xxx:******@127.0.0.1:5432/powerx?sslmode=disable&search_path=px_com_powerx_plugins_base
  schema: px_com_powerx_plugins_base
  user: pxu_com_powerx_plugins_base
  password: ******
...
- 安装插件时，宿主会依据插件 ID 生成唯一的 Schema / Database（前缀 `px_...`），并在宿主主库中执行 `CREATE SCHEMA IF NOT EXISTS`（PostgreSQL）或 `CREATE DATABASE IF NOT EXISTS`（MySQL）。
- 同时生成专用账号 `pxu_...` 与随机强密码，并仅授予对应 Schema 的 `CONNECT/USAGE/CREATE/SELECT/INSERT/UPDATE/DELETE` 权限；其余 Schema 不授予任何访问能力。
- `database.dsn` 会替换为插件专用账号，PostgreSQL 下追加 `search_path=<schema>`，MySQL 下直接指向隔离出来的数据库，确保插件默认只读写自己的 Schema。
- 卸载插件（含 `purge`）时，宿主会自动执行回收逻辑：删除专用 Schema / Database，并移除同名数据库用户，避免遗留无用账号。
