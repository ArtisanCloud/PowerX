...
> 用于阶段性冒烟（T058），覆盖 Provisioning → Ingestion → Fusion → Feedback 闭环。完成后请将结果附加到版本发布说明。

| 步骤 | 验证项 | 命令 / 链接 | 结果 |
| --- | --- | --- | --- |
| 1 | `go test ./tests/contract/knowledge_space` |  | ☐ |
| 2 | `go test ./tests/integration/knowledge_space` |  | ☐ |
| 3 | `npm run test:unit -- knowledge-spaces/ingestion.spec.ts` |  | ☐ |
...
| 7 | 反馈 API：`http POST :8080/api/admin/knowledge-spaces/<space>/feedback …` |  | ☐ |
| 8 | 验证 `reports/_state/knowledge-spaces.json` 既包含 `ingestion` 又包含 `feedback` 段 | `cat reports/_state/knowledge-spaces.json | jq '.'` | ☐ |
| 9 | Grafana Dashboards：`Knowledge Space`、`fusion-pipeline`、`feedback-loop` 均无红色告警 |  | ☐ |
| 10 | 将本次日期填入下表 |  | ☐ |
