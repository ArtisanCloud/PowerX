# STR-P3：KG 约束（关系驱动 / Knowledge Graph）

> 你要用 Knowledge Graph（知识图谱）做 RAG，就选 **P3**（前提是你真的有 KG 索引与可追溯 provenance）。

## 对应 RAG 策略模块（L3）

- 默认模块集合（见 `docs/guides/knowledge_space/scenary/MAP_scene_bundle_rag_profiles.md`）：`K_kg`, `H_fusion`, `C_context_enriched`, `L_feedback`
