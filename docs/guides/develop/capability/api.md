...
- **开放能力接口**：租户可访问的 API 均以 `{api_prefix}/*` 为前缀（由 `server.api_prefix` 配置，默认 `/api`，常见部署为 `/api/v1`），例如 `{api_prefix}/media/assets`、`{api_prefix}/tenant/invocations`。所有请求需要携带 `Authorization`，租户信息由 JWT claims 提供。
