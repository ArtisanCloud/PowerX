...
- **SSE / WebSocket**
  - `GET https: "//dev-api.powerx.local/internal/dev/plugins/stream`：事件类型包括 `SessionStarted`, `SessionReloaded`, `SessionTerminated`, `LogEntry`, `AlertRaised`；需携带 `Authorization` 与 `X-Tenant-UUID`。"
