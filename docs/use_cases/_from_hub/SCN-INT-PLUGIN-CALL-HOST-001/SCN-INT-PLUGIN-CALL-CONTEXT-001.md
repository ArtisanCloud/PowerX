...
2. **Invoke**：SDK 在请求 Header 中写入 `tenant_uuid`, `x-user-id`, `x-trace-id`, `x-permissions` 等字段，或在 JWT Claim 中携带。
...
- Header 规范：`tenant_uuid`, `x-user-id`, `x-plugin-id`, `x-trace-id`, `x-session-id`, `x-permissions`.
