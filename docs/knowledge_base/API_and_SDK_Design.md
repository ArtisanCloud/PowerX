...
- **多租户与安全**：强制 `X-Tenant-Id`，RBAC 最小权限，完整审计。
...
> 所有接口均要求：`Authorization` 与 `X-Tenant-Id`。
...
// 认证与多租户在网关或拦截器中处理：authorization/x-tenant-id
...
* 代码按第 4 章 proto 生成；拦截器统一注入 `authorization` 与 `x-tenant-id` 元数据
